...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create two-factor-257f
  fly apps create two-factor-257f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app two-factor-257f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app two-factor-257f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'two-factor-257f-staging' does not match app name in config file 'two-factor-257f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name two-factor-257f-db
  fly postgres attach --app two-factor-257f two-factor-257f-db

  fly postgres create --name two-factor-257f-staging-db
  fly postgres attach --app two-factor-257f-staging two-factor-257f-staging-db
  ```
